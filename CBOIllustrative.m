...
if spatial_plot
    legend([vstarplot], 'Global minimizer $v^*$','Location','northwest','Interpreter','latex','FontSize',12)
else
    legend([vstarplot], 'Global minimizer $v^*$','Location','northwest','Interpreter','latex','FontSize',12)
end
...
hold on
if spatial_plot
    legend([vstarplot, V_plot],'Global minimizer $v^*$','Particles $V_0^i$','Location','northwest','Interpreter','latex','FontSize',12)
else
    legend([vstarplot, V_plot],'Global minimizer $v^*$','Particles $V_0^i$','Location','northwest','Interpreter','latex','FontSize',12)
end
...
for k = 1:T/dt
    ...
    if show_expectation
        if spatial_plot
            legend([vstarplot, V_plot, Expectation_plot, valpha_plot], 'Global minimizer $v^*$', 'Particles $V_t^i$', 'Average particle $\textbf{E}\overline{V}_t$', 'Consensus point $v_{\alpha}(\rho_t^N)$','Location','northwest','Interpreter','latex','FontSize',12)
            %legend([vstarplot, V_plot, Expectation_plot, valpha_plot], 'Global minimizer $v^*$', 'Particles $V_t^i$', 'Average particle $\textbf{E}\overline{V}_t$', 'Consensus point $v_{\alpha}(\rho_t^N)$','Position',[0.175 0.675 0.1 0.2],'Interpreter','latex','FontSize',12))
        else
            legend([vstarplot, V_plot, Expectation_plot, valpha_plot], 'Global minimizer $v^*$', 'Particles $V_t^i$', 'Average particle $\textbf{E}\overline{V}_t$', 'Consensus point $v_{\alpha}(\rho_t^N)$','Location','northwest','Interpreter','latex','FontSize',12)
        end
    else
        if spatial_plot
            legend([vstarplot, V_plot, valpha_plot], 'Global minimizer $v^*$', 'Particles $V_t^i$', 'Consensus point $v_{\alpha}(\rho_t^N)$','Location','northwest','Interpreter','latex','FontSize',12)
            %legend([vstarplot, V_plot, valpha_plot], 'Global minimizer $v^*$', 'Particles $V_t^i$', 'Consensus point $v_{\alpha}(\rho_t^N)$','Position',[0.175 0.675 0.1 0.2],'Interpreter','latex','FontSize',12))
        else
            legend([vstarplot, V_plot, valpha_plot], 'Global minimizer $v^*$', 'Particles $V_t^i$', 'Consensus point $v_{\alpha}(\rho_t^N)$','Location','northwest','Interpreter','latex','FontSize',12)
        end
    end
